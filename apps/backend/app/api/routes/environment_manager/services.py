...
async def list_entries(ctx: WorkspaceContext, *, limit: int | None = None, offset: int = 0) -> list[EnvSetEntryOut]:
...
async def create_entry(ctx: WorkspaceContext, body: EnvSetEntryCreate) -> EnvSetEntryOut:
    eid = new_id()
    ts = create_timestamp()
...
# ponytail: cache removed during workspace refactor; re-add with (workspace_id, uid) key if hot
async def get_entry(*, ctx: WorkspaceContext, entry_id: str) -> EnvSetEntryOut:
    flt = apply_workspace_filter(ctx, {"_id": entry_id, "created_by": ctx.uid})
    doc = await db_manager.find_one(ENV_MANAGER_ENTRIES, flt)
    if not doc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Entry not found.")
    return _entry_doc_to_out(doc, entry_id=entry_id)


async def update_entry(ctx: WorkspaceContext, entry_id: str, body: EnvSetEntryUpdate) -> EnvSetEntryOut:
...
async def delete_entry(ctx: WorkspaceContext, entry_id: str) -> None:
    ...
    await safe_delete_one(
        ENV_MANAGER_ENTRIES, flt, name="Entry"
    )
