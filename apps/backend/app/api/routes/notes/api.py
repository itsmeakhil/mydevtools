...
async def get_note(note_id: str, ctx: WorkspaceContext = Depends(get_workspace_ctx)) -> NoteOut:
    return await note_svc.get_note(ctx=ctx, note_id=note_id)


@router.post("", response_model=NoteOut, summary="Create a note")
async def create_note(body: NoteCreate, ctx: WorkspaceContext = Depends(get_workspace_ctx)) -> NoteOut:
    return await note_svc.create_note(ctx, body)


@router.patch("/{note_id}", response_model=NoteOut, summary="Update a note (partial)")
async def patch_note(note_id: str, body: NoteUpdate, ctx: WorkspaceContext = Depends(get_workspace_ctx)) -> NoteOut:
