...
type Grid = number[][]
type Notes = boolean[][][]  // [row][col][digit-1]
...
function calcScore(timeSecs: number, level: Difficulty, hintsUsed: number): number {
...
function removeNoteFromPeers(notes: Notes, r: number, c: number, val: number): Notes {
...
export function SudokuLayout() {
  ...
  const saveScore = useCallback(async (timeSecs: number, hints: number) => {
