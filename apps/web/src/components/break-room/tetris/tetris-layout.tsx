...
type PieceDef = { shape: number[][]; color: string; idx: number }
type Piece = PieceDef & { x: number; y: number; rot: number }
type GameMode = 'marathon' | 'sprint' | 'ultra'
type TSpin = 'tspin' | 'tspin-mini' | null
...
function detectTSpin(board: Board, piece: Piece, lastWasRotation: boolean): TSpin {
  ...
  if (count >= 2) {
    // Front corners per rotation state
...
// Scoring with T-spin and B2B
const LINE_SCORE  = [0, 100, 300, 500, 800]
const TSPIN_SCORE = [400, 800, 1200, 1600]
const TMINI_SCORE = [100, 200, 400]

function calcScore(lines: number, level: number, tSpin: TSpin, b2b: boolean): { gain: number; label?: string } {
  let base: number
