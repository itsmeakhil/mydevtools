...
export function WhoisLookupLayout() {
  ...
  const lookup = useCallback(
    async (override?: string) => {
      ...
      setResult(null)
      try {
        const params = new URLSearchParams({ target: normalized.target })
        const res = await backendFetch(`/api/backend/whois/lookup?${params}`, {
          headers: { accept: 'application/json' },
        })
        if (!res.ok) {
          const body = await res.json().catch(() => ({}))
          throw new Error(body?.detail || t('errors.lookupFailed', { status: res.status }))
        }
