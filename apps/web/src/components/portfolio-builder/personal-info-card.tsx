...
export function PersonalInfoCard({ info, onChange }: PersonalInfoCardProps) {
  ...
  const filledFields = [
    info.phone, info.location, info.date_of_birth, info.nationality,
    ...(info.languages ?? []), ...(info.hobbies ?? []),
  ].filter(Boolean)
    ...
    <Card className="border shadow-sm bg-card/50 backdrop-blur-sm">
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <div className="space-y-1">
          ...
          <CardDescription>
            Contact details, languages, hobbies — everything a resume would include.
          </CardDescription>
        </div>
        {!isEditing && (
          <Button variant="ghost" size="sm" onClick={startEditing} className="h-8 gap-2 border text-xs">
          ...
          <div className="space-y-4 max-w-2xl">
            ...
            {(info.hobbies?.length ?? 0) > 0 && (
              <div className="space-y-1.5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {info.hobbies.map((h) => (
                    <Badge key={h} variant="outline" className="text-xs gap-1">
                      {h}
                    </Badge>
                  ))}
                </div>
