    ...
    <div className="group relative flex gap-3 rounded-lg border bg-background/50 p-4 transition-colors hover:bg-accent/20">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 mt-1 text-xs text-muted-foreground">
          ...
          <span>—</span>
          <span>{experience.endDate ? fmtMonth(experience.endDate) : '—'}</span>
        </div>
...
export function ExperienceBuilder({ experiences, onChange }: ExperienceBuilderProps) {
  ...
  const handleAdd = (exp: Experience) => {
    save([...experiences, exp])
    setIsAdding(false)
  }

  const handleEdit = (exp: Experience) => {
    save(experiences.map((e) => (e.id === exp.id ? exp : e)))
    setEditingId(null)
  }

  const handleDelete = (id: string) => {
    save(experiences.filter((e) => e.id !== id))
  }
    ...
    <Card className="border shadow-sm bg-card/50 backdrop-blur-sm">
      ...
      <CardContent className="pt-4 space-y-3">
        ...
        {experiences.length === 0 && !isAdding ? (
          ...
          experiences.map((exp) =>
            editingId === exp.id ? (
              <ExperienceForm key={exp.id} experience={exp} onSave={handleEdit} onCancel={() => setEditingId(null)} />
            ) : (
              <ExperienceItem key={exp.id} experience={exp} onEdit={() => setEditingId(exp.id)} onDelete={() => handleDelete(exp.id)} />
            )
          )
        )}
      </CardContent>
