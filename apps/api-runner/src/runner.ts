...
interface PreviousResponseSnapshot {
    status: number
    statusText: string
...
function walkRequests(items: (CollectionFolder | CollectionRequest)[], out: CollectionRequest[]): void {
    for (const it of items) {
        if ("type" in it && it.type === "folder") walkRequests(it.items, out)
        else out.push(it as CollectionRequest)
    }
}

function findRequestAncestors(
...
function mergeHeaders(chain: CollectionFolder[], requestHeaders: KeyValueItem[]): KeyValueItem[] {
    const seen = new Set<string>()
    const out: KeyValueItem[] = []
    for (const h of requestHeaders) { if (h.key) seen.add(h.key.toLowerCase()); out.push(h) }
...
function inherit(req: CollectionRequest, ancestors: CollectionFolder[]): CollectionRequest {
    if (ancestors.length === 0) return req
...
function parseResponsePath(s: string): (string | number)[] {
...
function resolveResponsePath(path: string, response: PreviousResponseSnapshot | null): string | undefined {
...
interface SubstituteArgs {
    ...
    previousResponse: PreviousResponseSnapshot | null
}

function makeSubstitute(args: SubstituteArgs) {
...
async function executeRequest(args: {
    ...
    previousResponse: PreviousResponseSnapshot | null
    signal?: AbortSignal
}): Promise<{ result: RequestRunResult; response: PreviousResponseSnapshot | null }> {
    ...
    if (req.preRequestScript && req.preRequestScript.trim()) {
        ...
        result.tests.push(...r.tests as TestResult[])
        result.logs.push(...r.logs as ScriptLog[])
    ...
    let urlObj: URL
    try { urlObj = new URL(substitute(workUrl)) } catch (e) {
    ...
    if (!noBody) {
        if (req.body.type === "json") {
            ...
            for (const it of req.body.formData ?? []) {
                if (!it.active || !it.key) continue
                if (it.valueType === "file") {
                    if (!it.fileContentBase64) continue
                    const buf = Buffer.from(it.fileContentBase64, "base64")
                    form.append(
                        substitute(it.key),
                        new Blob([buf], { type: it.fileType || "application/octet-stream" }),
                        it.fileName || "upload.bin",
                    )
                } else {
    ...
    const snapshot: PreviousResponseSnapshot = {
...
export async function runCollection(opts: RunOpts): Promise<RequestRunResult[]> {
    ...
    let previousResponse: PreviousResponseSnapshot | null = null

    for (let it = 0; it < iterations; it++) {
        const row = opts.dataRows?.[it] ?? {}
        for (const req of requests) {
            if (opts.abortSignal?.aborted) return out
            ...
            if (response) previousResponse = response
            const failedHere = result.networkError || result.tests.some((t) => !t.pass)
            if (failedHere && opts.bail) return out
        }
    }
