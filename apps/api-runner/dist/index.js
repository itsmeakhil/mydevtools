...
function parseArgs(argv) {
    const out = { iterations: 1, bail: false, help: false };
    ...
    while (i < argv.length) {
        ...
        if (a === "--data" || a === "-d") {
            ...
            out.iterations = Math.max(1, Number(argv[++i]) || 1);
        }
        else if (a === "--reporter" || a === "-r") {
            out.reporter = argv[++i];
        }
        else if (a === "--reporter-out" || a === "-o") {
            out.reporterOut = argv[++i];
        }
        else if (a === "--bail") {
            out.bail = true;
        }
        ...
        else if (!out.collectionPath) {
            out.collectionPath = a;
        }
        else
...
async function main() {
    ...
    if (args.cmd !== "run" || !args.collectionPath) {
        printUsage();
        process.exit(2);
    }
    ...
    for (const r of results) {
        ...
        if (failures.length === 0 && !r.networkError)
            continue;
